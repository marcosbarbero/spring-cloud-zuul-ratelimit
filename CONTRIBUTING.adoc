...
Spring Cloud Zuul RateLimit is released under the Apache 2.0 license.
...
* When writing a commit message please follow http://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html[these conventions], if you are fixing an existing issue please add `Fixes XXXX` at the end of the commit message (where `XXXX` is the issue number).
...
We use the
http://eclipse.org/m2e/[M2Eclipse] eclipse plugin for maven support.
